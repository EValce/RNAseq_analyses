...
Analysis of scRNAseq data produced by Jochen and Borja. RNA from Astrocytes, smNPC and DA neurons at 15, 30 and 50 days differentiated with Reinhardt protocol was extracted and sent for sequencing. The RPKM of specific genes were plotted. The heatmap of DA specific genes and neuronal precurson genes was created.
...
dat_seq_1<- readr::read_rds("fc_hs_q30RNA.rds")
...
```{r}
RNAseq_data %>% filter(gene_name %in% c("ALDH1A1", "TH","TUBB3", "SOX6","DCC","DDC", "MAPT", "KLHL1", "PBX1", "NR4A2", "FOXA2", "LMX1A", "SLC6A3", "STMN2", "SOX2","PITX3")) -> genes3

genes3 %>% filter(gene_name %in% c("ALDH1A1", "TH","TUBB3", "SOX6","DCC","DDC", "MAPT", "KLHL1", "PBX1", "NR4A2", "FOXA2", "LMX1A", "SLC6A3", "STMN2", "SOX2","PITX3")) -> genes3

genes3 %>% dplyr::select(2:8) %>% column_to_rownames(var = 'gene_name') -> genes3

pheatmap(genes3, annotation_col = info.data, show_colnames = TRUE, cluster_cols = FALSE)
